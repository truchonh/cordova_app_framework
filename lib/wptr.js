window.WebPullToRefresh = (function() {
    ...
    var defaults = {
        ...
        // ID of the element containing the page body
        bodyEl: 'body',
     ...
     * @param {Element|string} params.contentEl
     * @param {Element|string} params.bodyEl
     * @param {Element|string} params.ptrEl
     * @param {number} params.distanceToRefresh
     * @param {function} params.loadingFunction
     * @param {number} params.resistance
     */
    ...
    var _panDown = function(e) {
        if (!pan.enabled) {
            if (!pan.primed) {
                ...
                pan.enabled = true;
                options.bodyEl.style.overflow = 'hidden';
            }
        }

        options.ptrEl.querySelector('i').style.display = 'block';
        e.preventDefault();
        pan.distance = Math.abs(e.deltaY) / options.resistance;
    ...
    var _panUp = function(e) {
        if (!pan.primed) {
            return;
        } else if (pan.primed && !pan.enabled) {
            options.ptrEl.style.transform = options.ptrEl.style.webkitTransform = '';
            options.ptrEl.querySelector('i').style.display = 'none';
        ...
        let distance = Math.abs(e.deltaY);
        if ( pan.distance < distance / options.resistance ) {
            pan.distance = 0;
        } else {
            pan.distance = distance / options.resistance;
        }
    ...
    var _panEnd = function(e) {
        ...
        options.ptrEl.style.transform = options.ptrEl.style.webkitTransform = '';
        options.ptrEl.querySelector('i').style.display = 'none';
        options.bodyEl.style.overflow = 'scroll';
